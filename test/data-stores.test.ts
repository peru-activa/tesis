...
describe('centralized data-store contracts', () => {
  ...
  it('lists only orders linked to the requested quotations', async () => {
    const store = new MemoryOrderStore();
    await store.create({
      ...order,
      id: 'PED-R2-OWNED',
      source: { type: 'quotation', quotationId: 'COT-R2-OWNED', garmentIndex: 0 },
    });
    await store.create({
      ...order,
      id: 'PED-R2-FOREIGN',
      source: { type: 'quotation', quotationId: 'COT-R2-FOREIGN', garmentIndex: 0 },
    });

    assert.deepEqual(
      (await store.listBySourceQuotationIds(['COT-R2-OWNED'])).map((item) => item.id),
      ['PED-R2-OWNED'],
    );
    assert.deepEqual(await store.listBySourceQuotationIds([]), []);
  });
