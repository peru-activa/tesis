...
function rejectionReasons(request: RecommendationRequest, workshop: Workshop): string[] {
  ...
  if (availableDays < 0 || workshop.estimatedLeadTimeDays > availableDays) reasons.push('plazo insuficiente');
...
export function recommendWorkshops(request: RecommendationRequest): RecommendationResult {
  ...
  const candidates = eligible.map((workshop) => {
    const dimensions: DimensionScores = {
      delivery: Math.max(0, 1 - workshop.estimatedLeadTimeDays / availableDays),
    ...
    return {
      ...
      reasons: [
        `plazo estimado: ${workshop.estimatedLeadTimeDays} días`,
